...
export function useUploadResume() {
  ...
  return useMutation({
    mutationFn: ({ file, title, acknowledgeAi }) => resumesApi.upload(file, title, acknowledgeAi),
    onSuccess: (data) => {
...
export function useDeleteResume() {
  ...
  return useMutation({
    ...
    onSuccess: (_data, id) => {
      qc.removeQueries({ queryKey: resumeKeys.all, predicate: query => query.queryKey.includes(id) });
      qc.invalidateQueries({ queryKey: resumeKeys.list() });
      qc.invalidateQueries({ queryKey: dashboardKey });
      qc.invalidateQueries({ queryKey: ["analytics"] });
