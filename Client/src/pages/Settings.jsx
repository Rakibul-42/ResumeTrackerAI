...
import { useToast } from "@/context/ui-hooks";
...
function FieldLabel({ children, htmlFor }) {
  return (
    <label htmlFor={htmlFor} className="text-xs font-medium text-[var(--ink-muted)] mb-1.5 block">
      {children}
    </label>
  );
}
    ...
    <Card padding="lg" className="max-w-2xl">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">
            Your display name appears in your account greeting. Resume content is managed separately.
          </CardDescription>
        </div>
      </CardHeader>
      ...
      <form onSubmit={onSave} className="space-y-4">
        ...
        <div>
          ...
          <Input id="profile-name" autoComplete="nickname" required
            value={name}
...
function ThemeOption({ value, label, icon: Icon, current, onSelect }) {
  ...
  const descriptions = {
    light: "Bright cream and brown",
    dark: "Warm, low-glare night",
    "high-contrast": "Pitch-black OLED-friendly backgrounds",
  };
  return (
    <button
      type="button"
      ...
      aria-pressed={active}
      className={cn(
        "relative flex min-w-0 flex-col items-start gap-3 p-4 rounded-2xl border text-left transition-all",
        active
    ...
    <Card padding="lg" className="max-w-2xl">
      <CardHeader>
        <div>
          ...
          <CardDescription className="mt-1">
            Pick a theme. It is remembered on this device only when you allow optional storage.
          </CardDescription>
        </div>
      </CardHeader>
    ...
    <Card padding="lg" className="max-w-2xl">
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        <div>
          ...
          <Input id="current-password" required
            type="password"
        ...
        <div>
          ...
          <Input id="new-password" required aria-invalid={newTooShort} aria-describedby={newTooShort ? "new-password-error" : undefined}
            type="password"
            value={newPassword}
          ...
          />
          {newTooShort && (
            <p id="new-password-error" role="alert" className="text-[11px] text-[var(--danger)] mt-1.5">
        ...
        <div>
          ...
          <Input id="confirm-password" required aria-invalid={mismatch} aria-describedby={mismatch ? "confirm-password-error" : undefined}
            type="password"
            value={confirm}
          ...
          />
          {mismatch && (
            <p id="confirm-password-error" role="alert" className="text-[11px] text-[var(--danger)] mt-1.5">
