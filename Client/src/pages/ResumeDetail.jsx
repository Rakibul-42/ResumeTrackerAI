...
export default function ResumeDetail() {
  ...
  const activeVersionId = selection?.resumeId === id
    ? selection.versionId
    : resume?.currentVersionId || versions.at(-1)?._id || null;
  const setActiveVersionId = (versionId) => setSelection({ resumeId: id, versionId });
  ...
  if (isLoading) {
    return (
      <div className="space-y-4">
        <PageHeader title="Resume details" /><p role="status" className="sr-only">Loading resume</p>
  ...
  if (error) {
    return (
      <><PageHeader title="Resume details" /><EmptyState
        icon={FileText}
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-end gap-4 justify-between">
          <div className="space-y-2">
            ...
            <CardDescription>
              ...
              Scores are AI estimates, not actual ATS results. Review rewrites before exporting.
            </CardDescription>
            <VersionSwitcher
              versions={versions}
          ...
          <div className="flex flex-wrap items-end gap-3 flex-1 min-w-0 max-w-[520px]">
            <div className="min-w-0 flex-1 basis-48 space-y-2"><label htmlFor="target-role" className="text-sm font-medium">Target role (optional)</label>
            <Input id="target-role" maxLength={160}
      ...
      {analysisQuery.error && analysisQuery.error.code !== 'NO_ANALYSIS' && (
        <EmptyState icon={Sparkles} title="Couldn't load analysis" description={analysisQuery.error.message} />
      )}
      {!analysis && !analysisQuery.isLoading && (!analysisQuery.error || analysisQuery.error.code === 'NO_ANALYSIS') && (
        <EmptyState
          icon={Sparkles}
