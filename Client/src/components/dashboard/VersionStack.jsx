...
function DeltaIcon({ delta, ...props }) {
  if (delta > 0) return <TrendingUp {...props} />;
  if (delta < 0) return <TrendingDown {...props} />;
  return <Minus {...props} />;
}
...
export function VersionStack({ versions, resumeId, resumeTitle }) {
  ...
  const totalDelta = latest?.score != null && first?.score != null ? latest.score - first.score : null;
  ...
  return (
    <Card className="h-full flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_220px] gap-5 items-stretch">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          {visible.map((v, i) => {
            ...
            const delta = prev?.score != null && v.score != null ? v.score - prev.score : null;
        ...
        <div className="flex flex-col justify-between gap-4 lg:border-l lg:border-[var(--border)] lg:pl-5">
          ...
          <div className="space-y-2">
            ...
            >
              ...
              {totalDelta > 0 ? "+" : ""}
              {totalDelta == null ? "Not scored yet" : `${totalDelta} pts overall`}
            </div>
          </div>
      ...
      <div className="flex-1 mt-6 pt-5 border-t border-[var(--border)] space-y-5">
        ...
        {latest.score != null && <TierBar score={latest.score} />}
      </div>
    </Card>
  );
}
