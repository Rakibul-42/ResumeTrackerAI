...
export function UploadDropzone({ onUploaded, compact = false }) {
  ...
  async function submit() {
    if (!file || !acknowledgeAi) return;
    setErr("");
    try {
      const data = await upload.mutateAsync({ file, title, acknowledgeAi });
      setFile(null);
      setTitle("");
  ...
  return (
    <div className="space-y-3">
      {!file && (
        <div
          {...getRootProps({ role: 'button', 'aria-label': 'Choose a resume PDF, up to 5 MB' })}
          className={cn(
      ...
      {file && (
        <div className="space-y-3">
          <label htmlFor={titleId} className="block text-sm font-medium">Resume title (optional)</label>
          <Input
            id={titleId}
            maxLength={160}
          ...
          <label className="flex items-start gap-3 text-sm leading-relaxed">
            <input type="checkbox" checked={acknowledgeAi} onChange={event => setAcknowledgeAi(event.target.checked)} className="mt-1 h-5 w-5 shrink-0 accent-[var(--accent)]" />
            <span>I understand that resume text is sent to Google Gemini for processing and stored as described in the <Link className="underline" to="/privacy">privacy notice</Link>.</span>
          </label>
          <Button
            onClick={submit}
            variant="accent"
            size="lg"
            disabled={upload.isPending || !acknowledgeAi}
            className="w-full"
          >
            {upload.isPending ? (
              <>
      ...
      {err && (
        <div role="alert" className="text-sm text-[var(--danger)] bg-[var(--surface-2)] rounded-xl px-3 py-2">
          {err}
        </div>
      )}
    </div>
