...
function readLastSeen() {
  ...
  const raw = readOptional(browserStorage(), LAST_SEEN_KEY);
...
export function NotificationsPopover() {
  ...
  function toggle() {
    if (!open && events.length) {
      ...
      writeOptional(browserStorage(), LAST_SEEN_KEY, String(newest));
      setLastSeen(newest);
    }
    setOpen((v) => !v);
  }
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute right-0 top-[52px] z-40 w-[min(380px,calc(100vw-32px))] rounded-3xl bg-[var(--surface)] border border-[var(--border)] shadow-hover overflow-hidden"
            role="dialog"
