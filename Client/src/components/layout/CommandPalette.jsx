...
export function CommandPalette({ open, onClose }) {
  return <AnimatePresence>{open && <PaletteContent onClose={onClose} />}</AnimatePresence>;
}

function PaletteContent({ onClose }) {
  ...
  function handleKeyDown(e) {
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setActiveIdx((i) => Math.max(0, Math.min(items.length - 1, i + 1)));
    } else if (e.key === "ArrowUp") {
      e.preventDefault();
      setActiveIdx((i) => Math.max(0, i - 1));
    } else if (e.key === "Enter") {
      e.preventDefault();
      const it = [...navMatches, ...resumeMatches][activeIdx];
      if (it) {
        navigate(it.to);
        onClose();
      }
  ...
  return (
    <AnimatePresence>
      {(
        <dialog ref={dialogRef} aria-label="Command palette" aria-modal="true"
          onCancel={(event) => { event.preventDefault(); onClose(); }}
          className="fixed inset-0 m-0 h-full max-h-none w-full max-w-none border-0 bg-transparent text-[var(--ink)] open:flex items-start justify-center pt-[14vh] px-4"
        >
          <div
          ...
          >
            <div className="flex items-center gap-3 px-5 h-14 border-b border-[var(--border)]">
              ...
              <input
                ref={inputRef}
                aria-label="Search resumes and pages"
                value={query}
                onChange={(e) => { setQuery(e.target.value); setActiveIdx(0); }}
                onKeyDown={handleKeyDown}
                placeholder="Search resumes or jump to a page..."
                className="min-w-0 flex-1 bg-transparent outline-none text-sm text-[var(--ink)] placeholder:text-[var(--ink-muted)]"
              />
              <button type="button" aria-label="Close search" onClick={onClose} className="h-11 w-11 shrink-0 flex items-center justify-center rounded-full hover:bg-[var(--surface-2)]"><X size={18} /></button>
            ...
            <div ref={listRef} className="max-h-[52vh] overflow-y-auto p-2">
              ...
              {resumeMatches.length > 0 && (
                <div className="mt-1">
                  ...
                  <div className="flex flex-col gap-0.5">
                    {resumeMatches.map((it, idx) => renderItem(it, navMatches.length + idx))}
                  </div>
                </div>
              )}
            </div>
