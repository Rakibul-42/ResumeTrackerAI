  ...
  return (
    <aside
      className={cn(
        ...
        "w-[88px] hover:w-[248px] focus-within:w-[248px]",
...
export function MobileNavigation() {
  ...
  return (
    <nav aria-label="Mobile navigation" className="md:hidden flex flex-wrap gap-2 mb-6 pb-4 border-b border-[var(--border)]">
      {[...NAV, { to: '/settings', label: 'Settings', icon: Settings }].map(({ to, label, icon: Icon }) => (
        <NavLink key={to} to={to} className={({ isActive }) => cn(
          'inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-medium',
          isActive ? 'bg-[var(--ink)] text-[var(--bg)]' : 'bg-[var(--surface)] text-[var(--ink)]'
        )}>
          <Icon size={16} aria-hidden="true" />{label}
        </NavLink>
      ))}
      <button type="button" onClick={logout} className="inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm bg-[var(--surface)]">
