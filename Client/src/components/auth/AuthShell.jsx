...
export function AuthShell({ children, headline, subhead }) {
  return (
    <div className="min-h-screen flex bg-[var(--bg)] p-3 sm:p-4 gap-0 lg:gap-4">
      {/* Left — form */}
      <main id="main-content" tabIndex={-1} className="min-w-0 flex-1 flex items-center justify-center px-4 sm:px-8 py-10">
  ...
  return (
    <div className="hidden lg:block flex-1 relative rounded-[28px] overflow-hidden isolate">
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(140deg, var(--brand-deep) 0%, var(--brand-brown) 38%, var(--brand-deep) 72%, #160d04 100%)",
        }}
      />
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, color-mix(in srgb, var(--brand-cream) 55%, transparent) 0%, transparent 70%)",
          filter: "blur(60px)",
        }}
        animate={{
          x: [0, 40, 0],
      ...
      <motion.div
        ...
        style={{
          background:
            "radial-gradient(circle, color-mix(in srgb, var(--brand-orange) 60%, transparent) 0%, transparent 70%)",
          filter: "blur(60px)",
        }}
        animate={{
          x: [0, -30, 0],
      ...
      <div className="relative z-10 h-full flex flex-col justify-between pt-10 xl:pt-16">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-[var(--brand-cream)]/8 border border-[var(--brand-cream)]/10 backdrop-blur-md">
            <Sparkles size={12} className="text-[var(--brand-cream)]/80" />
            <span className="text-[11px] tracking-wide text-[var(--brand-cream)]/80 uppercase font-semibold">
              AI Resume Tracker
            </span>
          </div>

          <h2
            className="font-serif text-[44px] xl:text-[60px] leading-[1.02] text-[var(--brand-cream)] mt-8 max-w-[540px]"
            style={{
              fontStyle: "italic",
              fontWeight: 500,
          ...
          <p className="text-[var(--brand-cream)]/65 text-base xl:text-lg mt-6 max-w-md leading-relaxed">
            {subhead}
          </p>
        </motion.div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-1.5">
        <label htmlFor={inputId} className="text-sm font-medium text-[var(--ink)]">{label}</label>
        {extra}
      </div>
...
export function AuthPrimaryButton({ children, disabled, ...props }) {
  return (
    <motion.button
      ...
      disabled={disabled}
      className="relative w-full h-12 rounded-2xl text-[var(--brand-cream)] font-semibold text-[15px] flex items-center justify-center gap-2 overflow-hidden shadow-[0_8px_24px_-8px_rgba(192,88,0,0.55)] transition-all duration-200 hover:shadow-[0_12px_28px_-8px_rgba(192,88,0,0.7)] disabled:opacity-60 disabled:cursor-not-allowed"
      style={{
        background:
...
export function AuthErrorBanner({ children }) {
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      className="text-sm text-[var(--danger)] bg-[var(--surface-2)] border border-current rounded-2xl px-4 py-2.5 leading-snug"
    >
      {children}
    </motion.div>
  );
